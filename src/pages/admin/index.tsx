...
const cards = [
    {
        value: 'edit-challenges',
        label: 'Editer les challenges',
        description: 'Ajouter ou modifier un challenge.',
        id: 1
    },
    {
        value: 'manage-users',
...
export default function Challenges({ customSession }: ProfileProps) {
    return (
        <>
            <Head>
                <title>Challenges Hub - Administration</title>
            </Head>
            <main className="bg-gray-900 text-white min-h-screen">
                <NavBar customSession={customSession} />
                <section className="container py-16 mx-auto px-4">
                    <h1 className="text-5xl font-bold mb-6 text-center">Administration</h1>
...
export async function getServerSideProps(context: any) {
    ...
    if (!session && customSession.user?.permRole !== 'owner') {
        return {
            redirect: {
                destination: '/',
                permanent: false
